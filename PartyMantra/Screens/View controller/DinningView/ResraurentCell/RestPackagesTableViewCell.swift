//
//  RestPackagesTableViewCell.swift
//  PartyMantra
//
...
protocol GetRestComboHeightOfProtocol {
    func getComboHeightFromArr(height:Int,restCellHeight:Int,isViewMoreBtnClicked:Bool,indexInt:Int) -> Void
   }

class RestPackagesTableViewCell: UITableViewCell,UITableViewDelegate,UITableViewDataSource {



       
        var comboDelegate : GetRestComboHeightOfProtocol?
        var heightArr : [Int] = []
        ...
        func configureRestPackageCell(modal:RestaurantInfoModel?) -> Void {
        ...
        guard let cell = sender.superview?.superview?.superview as? ViewMoreTableViewCell else {
            ...
            comboDelegate?.getComboHeightFromArr(height: self.newHeight+68,restCellHeight:self.heightOfCell+100, isViewMoreBtnClicked: self.isViewMoreClicked, indexInt: sender.tag)
